...
local function HandleFruit(v)
    ...
    task.spawn(function()
        ...
        while v.Parent ~= nil and h.Parent ~= nil do
            ...
            local char = LP.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            local isHeld = v:FindFirstAncestorOfClass("Model")
                        and v:FindFirstAncestorOfClass("Model"):FindFirstChild("Humanoid")

            if root and not isHeld and CollectingCount == 0 then
                if _G.Settings.MagnetFruit and not _G.Settings.TPCollect then
                    if (root.Position - h.Position).Magnitude < 80 then
